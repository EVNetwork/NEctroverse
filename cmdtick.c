...
if( artsnum == ARTEFACT_NUMUSED ) {
    ...
    if( ( 3*artsnum >= ARTEFACT_NUMUSED ) || ( (3*dbArtefactMax)/2 >= ARTEFACT_NUMUSED ) )
    {
      ...
      {
        if( empirep[stats[a+0]].artefacts & d )
          fprintf( file, "<img src=\"http://www.nectroverse.net:8080/images/%s\">", artefactImage[c] );
      }
    }
...
if( artsnum ) {
	...
	for( c = 0 ; c < ARTEFACT_NUMUSED ; c++ ) {
		if( artefacts[c] )
			fprintf( file, "<img src=\"http://www.nectroverse.net:8080/images/%s\"> %s<br>", artefactImage[c], artefactDescription[c] );
	}
