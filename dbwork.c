...
unsigned char dbFileUsersName[] = "users";
...
unsigned char dbFileUserUserName[] = "user%d/user";
unsigned char dbFileUserMainName[] = "user%d/main";
unsigned char dbFileUserBuildName[] = "user%d/build";
unsigned char dbFileUserPlanetsName[] = "user%d/planets";
unsigned char dbFileUserFleetsName[] = "user%d/fleets";
unsigned char dbFileUserNewsName[] = "user%d/news";
unsigned char dbFileUserMarketName[] = "user%d/market";
unsigned char dbFileUserMailInName[] = "user%d/mailin";
unsigned char dbFileUserMailOutName[] = "user%d/mailout";
unsigned char dbFileUserSpecOpsName[] = "user%d/specops";
unsigned char dbFileUserRecordName[] = "user%d/record";


#define DB_FILE_USER_NUMBER 11
...
unsigned char *dbFileUserList[DB_FILE_USER_NUMBER] = { dbFileUserUserName, dbFileUserMainName, dbFileUserBuildName, dbFileUserPlanetsName, dbFileUserFleetsName, dbFileUserNewsName, dbFileUserMarketName, dbFileUserMailInName, dbFileUserMailOutName, dbFileUserSpecOpsName, dbFileUserRecordName };

long long int dbFileUserListDat0[] = { 0, -1, -1, 0, 0 };
int dbFileUserListDat1[] = { 0, 8 };

int dbFileUserListBase[DB_FILE_USER_NUMBER] = { 0, 0, 4, 4, 4, 40, 8, 8, 8, 4, 4 };
long long int *dbFileUserListData[DB_FILE_USER_NUMBER] = { 0, 0, dbFileUserListDat0, dbFileUserListDat0, dbFileUserListDat0, dbFileUserListDat0, dbFileUserListDat0, dbFileUserListDat1, dbFileUserListDat1, dbFileUserListDat0, dbFileUserListDat0 };
...
FILE *dbFileGenOpen( int num )
{
  ...
  if( !( dbFilePtr[num] = fopen( dbFileList[num], "rb+" ) ) )
  {
...
FILE *dbFileUserOpen( int id, int num )
{
  ...
  FILE *file;
  sprintf( fname, dbFileUserList[num&0xFFFF], id );
...
int dbUserAdd( unsigned char *name, unsigned char *faction, unsigned char *forumtag )
{
  int a, id, freenum;
  dbUserPtr user;
  unsigned char dname[32], fname[32];
  dbUserDescDef descd;
  FILE *file;
  ...
  sprintf( dname, "user%d", id );
  if( mkdir( dname, S_IRWXU ) == -1 )
  {
    dbUserFree( user );
    printf( "Error %02d, mkdir %d\n", errno, id );
    return -3;
  }


  for( a = DB_FILE_USER_NUMBER-1 ;  ; a-- )
  {
    sprintf( fname, dbFileUserList[a], id );
    if( !( file = fopen( fname, "wb+" ) ))
    {
      dbUserFree( user );
      rmdir( dname );
  ...
  sprintf( user->faction, faction );
  sprintf( user->forumtag, forumtag );
  user->id = id;
  user->flags = 0;
  user->level = 0;
...
int dbUserRemove( int id )
{
  ...
  for( a = 0 ; a < DB_FILE_USER_NUMBER ; a++ )
  {
    sprintf( fname, dbFileUserList[a], id );
    unlink( fname );
  }
