...
char * currentIp;
if (iohtmlHeaderFind( cnt, "X-Forwarded-For" ) != NULL){
	currentIp = strdup(iohtmlHeaderFind( cnt, "X-Forwarded-For" ));
	if (inet_aton(currentIp, &sin_addrTmp) == 0){
		sin_addrTmp = ((struct sockaddr_in *)(cnt->connection)->addr)->sin_addr;
	}
	free(currentIp);
}
else
...
if( ( ( forumstring ) && ( sscanf( forumstring, "%d", &forum ) == 1 ) ) ) {
 ...
 if( ( flags ) && ( !( (cnt->session)->dbuser ) || !( ((cnt->session)->dbuser)->level >= LEVEL_MODERATOR ) ) ) {
  ...
  httpPrintf( cnt, "<style>  p a:link {color: #ff9900;}  </style>");
...
if( ( ( id != -1 ) && ( ( flags == false ) || ( ( flags == true ) && ( forum != maind.empire ) ) ) ) ) {
  ...
  strftime(timebuf,512,"%a, %d %b %G %T %Z", gmtime( &forums[a].time ) );
   httpPrintf( cnt, "<tr bgcolor=\"#111111\"><td><p><a href=\"%s%s&amp;forum=%d&last=%d\">%s</a><p></td><td>%d</td><td nowrap>", URLAppend( cnt, "forum" ), ( flags ? "&amp;empire=true" : "" ), a, forums[a].time, forums[a].title, forums[a].threads );
...
if( flags == false ) {
  ...
  httpPrintf( cnt, "<style>  xxx a:link {color: #ff9900;}  </style>");
for( a = 0 ; a < b ; a++ ) {
   ...
   httpPrintf( cnt, "<p><tr bgcolor=\"#111111\"><td><xxx><a href=\"%s%s&amp;forum=%d&thread=%d&last=%d\">%s</a></xxx>", URLAppend( cnt, "forum" ), ( flags ? "&amp;empire=true" : "" ), forum, threads[a].id, threads[a].time, threads[a].topic );
  ...
  {
   if (maind.benefactor[4] != 1  && id != -1){ //if the user is allowed to post
   ...
   httpString( cnt, "<table cellspacing=\"3\" class=\"left\"><tr><td>Name</td><td>" );
	if( id == -1 ) {
		...
		httpString( cnt, "</td>Only registered users can post!<td>" );
	} else {
   ...
   if( badpost ) {
   	...
   	httpPrintf( cnt, "<tr><td>Post</td><td><textarea name=\"post\" style=\"height:200px\" wrap=\"soft\" rows=\"10\" cols=\"60\">%s</textarea></td></tr>", ( poststring ) ? poststring : ""  );
   } else {
   	httpString( cnt, "</td></tr><tr><td>Topic</td><td><input type=\"text\" name=\"topic\" size=\"32\"></td></tr>" );
   	httpString( cnt, "<tr><td>Post</td><td><textarea name=\"post\" style=\"height:200px\" wrap=\"soft\" rows=\"10\" cols=\"60\"></textarea></td></tr>" );
   }
   httpString( cnt, "<tr><td>&nbsp;</td><td class=\"left\"><input type=\"submit\" value=\"Post\"></td></tr></table></form>" );
	...
	httpString( cnt, "You were banned from writing on the forum!");
	}
  }
...
settings = GetSetting( "Server Name" );
  httpPrintf( cnt, "<table cellspacing=\"4\" width=\"%d%%\" class=\"left\">", ( id == -1 ) ? 100 : 80 );
  httpPrintf( cnt, "<p><tr><td><a href=\"%s\" target=\"_top\">%s</a> - ", URLAppend( cnt, "index" ), settings->string_value );
  httpPrintf( cnt, "<a href=\"%s\">%s public forums</a> - ", URLAppend( cnt, "forum" ), settings->string_value );
  httpPrintf( cnt, "<a href=\"%s%s&amp;forum=%d\">%s</a> - %s</td></p><td class=\"right\">", URLAppend( cnt, "forum" ), ( flags ? "&amp;empire=true" : "" ), forum, forumd.title, threadd.topic );
if( flags == false )  {
	time( &tint );
	strftime(timebuf,512,"%a, %d %b %G %T %Z", gmtime( &tint ) );
	httpString( cnt, timebuf );
  ...
  if( iohttpForumPerms( flags, id, forum, cnt, &maind, forumd.wperms ) )
  {
	...
	if (maind.benefactor[4] != 1 && id != -1){  //if user is allowed to post
   httpPrintf( cnt, "<form action=\"%s\" method=\"POST\">", URLAppend( cnt, "forum" ) );
   httpPrintf( cnt, "<input type=\"hidden\" name=\"empire\" value=\"%s\">", ( flags ? "true" : "false" ) );
   httpPrintf( cnt, "<input type=\"hidden\" name=\"forum\" value=\"%d\">", forum );
 ...
 else if( action == 3 )
 {
  ...
  threadd.flags = 0;
	...
	memcpy( &(threadd.sin_addr), &sin_addrTmp , sizeof(struct in_addr) );
...
if( id < 0 ) {
  ...
  //memcpy( &(postd.post.sin_addr), &( ((struct sockaddr_in *)(cnt->connection)->addr)->sin_addr ), sizeof(struct in_addr) );
	memcpy( &(postd.post.sin_addr), &sin_addrTmp , sizeof(struct in_addr) );
 ...
 else if( action == 7 )
 {
  ...
  httpString( cnt, "Edit post<br><br><textarea name=\"post\" style=\"height:200px\" wrap=\"soft\" rows=\"10\" cols=\"60\">" );
