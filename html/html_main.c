...
if( (id < 0) ) {
	...
	httpString( cnt, "<br>" );
	if( bitflag( ((cnt->session)->dbuser)->flags, CMD_USER_FLAGS_ACTIVATED ) ) {
		httpPrintf( cnt, "<a href=\"%s\" target=\"%s\">Proceed to game</a><br>", URLAppend( cnt, "main" ), ( iohtmlVarsFind( cnt, "fbapp" ) == NULL ) ? "_top" : "top" );
	} else {
		httpPrintf( cnt, "<a href=\"%s\" target=\"%s\">Activate Now!</a><br>", URLAppend( cnt, "register" ), ( iohtmlVarsFind( cnt, "fbapp" ) == NULL ) ? "_top" : "top" );
	}
	httpString( cnt, "<br>" );
	httpPrintf( cnt, "<a href=\"%s\" target=\"%s\">Log out</a><br>", URLAppend( cnt, "logout" ), ( iohtmlVarsFind( cnt, "fbapp" ) == NULL ) ? "_top" : "top" );
	httpString( cnt, "<br>" );
	httpString( cnt, "<br>" );
}
