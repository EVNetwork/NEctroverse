...
if( ( id >= 0 ) && ( user ) && ( ( curfam == maind.empire ) || ( ( (cnt->session)->dbuser ) && ( ((cnt->session)->dbuser)->level >= LEVEL_MODERATOR ) ) ) ) {
 ...
 if( empired.artefacts )
 {
  ...
  {
   c = 0;
   for( a = 0, b = 1 ; a < ArtefactNum ; a++, b <<= 1 )
   {
    if( empired.artefacts & b )
     c++;
   }
  }
  if( ( 3*c >= ArtefactNum ) || ( (3*dbArtefactMax)/2 >= ArtefactNum ) )
  {
 ...
 for( y = basey ; y < endy ; y++ )
 {
  ...
  for( x = basex ; x < endx ; x++, i++ )
  {
   ...
   else
   {
    ...
    httpPrintf( cnt, "<img border=\"0\" src=\"files?type=image&amp;name=map/m%c%c.gif\" title=\"%d,%d\"></a>", a, b, x, y );
   }
  }
  if( zoomsize >= 0 )
...
if( ( id = iohtmlIdentify( cnt, 1|2 ) ) < 0 )
 ...
 httpPrintf( cnt, "<a href=\"%s&amp;e0=3&amp;c0=2&amp;e1=1&amp;c1=1&amp;e2=2&amp;c2=0&amp;setdefault=1&amp;request=true\" rel=\"ajaxpanel\" data-loadtype=\"ajax\">Your empire in red, your planets in blue and portals in green</a><br>", URLAppend( cnt, "map" ) );
 httpString( cnt, "</td></tr></table>" );
