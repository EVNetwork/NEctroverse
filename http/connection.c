...
const char *
MHD_print_connections_values (struct MHD_Connection *connection,
                             enum MHD_ValueKind kind)
{
  struct MHD_HTTP_Header *pos;
  
  char * tmp, * new_str = "", * tmp2, *tmp3;
  int checkMalloc = 0;

  if (NULL == connection)
    return NULL;
  for (pos = connection->headers_received; NULL != pos; pos = pos->next){
    if ((pos != NULL ) && (0 != (pos->kind & kind)) && ( NULL != pos->header) && (pos->value != NULL)){
			tmp = strdup(pos->value);
			tmp3 = strdup(pos->header);
			tmp2 = new_str;

			if((new_str = malloc(strlen(tmp3)+strlen(tmp2)+strlen(tmp)+1)) != NULL){
