...
#define SERVERALLOCATION (1024 * KB_SIZE)
...
struct MHD_OptionItem ops[] = {
	...
	{ MHD_OPTION_PER_IP_CONNECTION_LIMIT, (unsigned int)(240), NULL },
	{ MHD_OPTION_CONNECTION_TIMEOUT, (unsigned int)(524), NULL },
...
if( ( strncmp(url,"/",1) != CMP_TRUE ) && ( iohtmlVarsFind( cnt, "cookoff" ) != NULL ) && ( strstr( url, (cnt->session)->sid ) == 0 ) ) {
	if( ServerSessionMD5 == NULL ) {
		if( GenServerSum() == NO ) {
	...
	AddBufferString( urlappend_buffer, "?2fix" );
}
...
/*
void a_special (char const * caller_name )  //bug search
{
	printf(" WWW was called from %s", caller_name);
	WWWExpire();
}


#define WWWExpire() a_special(__func__)
*/
