...
ConfigListArrayDef config_listing[] = {
	...
	{ INI_TYPE_DOUBLE, "map:ancient_civ", "0", "Map Ancient civillization Bonus" },
...
for( a = 0; a < num; a++ ) {
	...
	snprintf(buffer, sizeof(buffer), "admin%d:email", a+1 );
	strcpy( array[a].email, iniparser_getstring(config, buffer, "" ) );
	
	snprintf(buffer, sizeof(buffer), "admin%d:ip", a+1 );
	strcpy( array[a].ip, iniparser_getstring(config, buffer, "" ) );
...
iniparser_set(ini,"admin1:email","admin@ev.com");
iniparser_set(ini,"admin1:faction", "Admins Faction1" );
